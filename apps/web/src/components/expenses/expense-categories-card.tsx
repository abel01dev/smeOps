...
export function ExpenseCategoriesCard() {
  ...
  const [renameTarget, setRenameTarget] = React.useState<ExpenseCategory | null>(
    null,
  );
        ...
        <CardContent className="space-y-4">
          <form onSubmit={onAdd} className="flex flex-col gap-2 sm:flex-row">
            <div className="flex-1">
              <Label htmlFor="new-exp-cat" className="sr-only">
                {t("newCategory")}
              </Label>
              <Input
                id="new-exp-cat"
              ...
              {list.map((c) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    {typeof c.expenseCount === "number" && (
                      <p className="text-xs text-muted-foreground">
                        {t("categoryExpenseCount", { count: c.expenseCount })}
                      </p>
                    )}
                  </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={onRename} className="grid gap-4">
            <div className="grid gap-2">
              <Label htmlFor="rename-exp-cat">{t("categoryName")}</Label>
              <Input
                id="rename-exp-cat"
                className="h-11"
                disabled={busy}
